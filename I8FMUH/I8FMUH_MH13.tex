...
\subsubsection{\quad 1) Nedves-gőz állapotjelzők $x$ = 0,7 és $p$ = 1,76 bar esetén:}
\begin{equation}
	p_1=\SI{1,76}{\bar},
	\quad
...
\subsubsection{\quad 2) Telített gőz vonalon az állapotjelzők:}
\begin{equation}
	p"=\SI{2,575}{\bar},
	\quad
	T"=\SI{401,52}{\kelvin},
...
\subsubsection{\quad 3) Túlhevített gőz mezőnél a 2-es pontban az állapotjelzők:}
\begin{equation}
	p_2=\SI{3,073}{\bar},
	\quad
...
\begin{equation}
	q_{12}=u_2-u_1=\SI{2649,5}{\frac{\kilo\joule}{\kilogram}}-\SI{1913,2}{\frac{\kilo\joule}{\kilogram}}=\SI{736,3}{\frac{\kilo\joule}{\kilogram}}
\end{equation}
...
\begin{figure}[h]
	...
	xlabel={},
	ylabel={},
	ytick={\TegyC, \TvesszoC, \TkettoC},
	yticklabels={$T_1$, $T"$, $T_2$},
	xtick={\segy, \svesszo, \sketto},
	xticklabels={$s_1$, $s"$, $s_2$},
	...
	\node[thick, circle, draw, inner sep = 1mm] at (axis cs:4.9, 85) {1};
...
\begin{figure}[h]
	...
	\newcommand\kicsinyites{0.6};
	% Tengelyek
	\draw[myarrow] ({0}, {0}) -- ({14*\kicsinyites}, {0}) node[anchor=base east, shift={(0,-0.5)}]{$log(v)$};
	\draw[myarrow] ({0}, {0}) -- ({0}, {9*\kicsinyites}) node[anchor=north east]{$p$};
	...
	\node[thick, circle, draw, inner sep = 1mm] at (axis cs:0.75, 0.176) {1};
	\node[thick, circle, draw, inner sep = 1mm] at (axis cs:0.75, 0.32) {2};
