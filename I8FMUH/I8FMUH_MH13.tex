...
\end{equation}
\subsection*{d) Az állapotváltozás $T-s$ és $p-v$ diagramban}
%T-s diagram
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
	%sraffozás beállításai
		...
		\begin{axis}[
			...
			%T-s alap
			\addplot [thick] table {./I8FMUH/ts.txt}; %T-s diagram alap
			\addplot [thick, red] table {./I8FMUH/ts_v0699.txt}; %v=0.699 vonal ábrázolása
			...
			\node[thick, circle, draw, inner sep = 1mm] at (axis cs:4.9, 60) {1};
			\node[thick, circle, draw, inner sep = 1mm] at (axis cs:8, 200) {2};
			...
			%fajtérfogat görbe jelölése
			\node[red, rotate = 77] at (axis cs:7.04, 370) {\SI{0,699}{\meter\cubed\per\kilogram}};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{axis}[
			...
			xtick = {\vegy, 0.7},
			xticklabels = {{$0,699$}, {}}, %sajnos a vesszőt csak úgy tudtam megoldani, hogy a listához hozzátettem egy értéket, ami "láthatatlan"
			xmode = log,
			width=15cm*\kicsinyites, height=10cm*\kicsinyites, xmin=0, ymin=0
			]
